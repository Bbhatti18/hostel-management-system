              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? <tr><td colSpan={13} className="px-5 py-12 text-center text-sm text-slate-500">Loading your bills...</td></tr> : filteredBills.length === 0 ? <tr><td colSpan={13} className="px-5 py-12 text-center text-sm text-slate-500">No bills found.</td></tr> : filteredBills.map((bill) => (
                  <tr key={bill.id}>
                    ...
                    <td className="px-4 py-4"><div className="flex gap-2"><button type="button" onClick={() => void openBill(bill.id)} className="rounded-lg border border-indigo-200 px-3 py-2 text-xs font-semibold text-indigo-700">View</button><button type="button" onClick={() => void openBill(bill.id, true)} className="rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700">Print</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
